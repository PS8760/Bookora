...
export default function LoginPage() {
  ...
  const handleGithub = async () => {
    ...
    try {
      await signIn.social({ provider: "github", callbackURL: "/api/auth/redirect" });
    } catch {
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl border border-[#E8E0D0] shadow-[0_8px_40px_rgba(114,74,106,0.10)] p-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
            {/* Google OAuth */}
            <button type="button" onClick={handleGoogle} disabled={googleLoading || githubLoading}
              className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl border border-[#E8E0D0] bg-[#FFFBE9] text-sm font-medium text-[#4A4A6A] hover:border-[#724A6A] hover:text-[#724A6A] transition-colors disabled:opacity-60">
              {googleLoading ? (
            ...
            {/* GitHub OAuth */}
            <button type="button" onClick={handleGithub} disabled={githubLoading || googleLoading}
              className="flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl border border-[#E8E0D0] bg-[#FFFBE9] text-sm font-medium text-[#4A4A6A] hover:border-[#724A6A] hover:text-[#724A6A] transition-colors disabled:opacity-60">
              {githubLoading ? (
                <svg className="animate-spin" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M21 12a9 9 0 1 1-6.219-8.56" />
