          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  { value: "customer",  label: "Customer",  icon: <User size={24} strokeWidth={1.5} />,      desc: "Book appointments" },
                  { value: "organiser", label: "Organiser", icon: <Building2 size={24} strokeWidth={1.5} />, desc: "Manage services" },
                ].map((r) => (
                  <button
                    key={r.value}
                    type="button"
                    onClick={() => setForm({ ...form, role: r.value })}
                    className={`flex flex-col items-center p-3 rounded-xl border-2 transition-all text-center ${form.role === r.value
                        ? "border-[#724A6A] bg-[#F5EDF4]"
                        : "border-[#E8E0D0] bg-[#FFFBE9] hover:border-[#D4B8CF]"
                      }`}
                  >
                    <span className="mb-1 text-[#724A6A]">{r.icon}</span>
