...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="h-screen bg-[#FFFBE9] flex flex-col overflow-hidden">
      <header className="h-14 flex-shrink-0 bg-[#FFFBE9] border-b border-[#E8E0D0] flex items-center px-4 gap-4 sticky top-0 z-40">
        <button className="lg:hidden p-2 rounded-lg hover:bg-[#F5EDF4] text-[#1A1A2E]" onClick={() => setSidebarOpen(!sidebarOpen)}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <line x1="3" y1="6" x2="21" y2="6" /><line x1="3" y1="12" x2="21" y2="12" /><line x1="3" y1="18" x2="21" y2="18" />
          </svg>
        </button>
<Link href="/" className="flex items-center gap-2 h-full justify-center">
           <BookoraLogo height={40} linked={false} />
         </Link>
        <span className="badge bg-[#F5EDF4] text-[#724A6A] border border-[#D4B8CF] text-[10px]">Admin</span>
...
<div className="flex items-center gap-2">
           ...
           {user?.image ? (
             <img src={user.image} alt={user.name} className="w-7 h-7 rounded-full object-cover border-2 border-[#E8E0D0]" />
           ) : (
             <div className="w-7 h-7 rounded-full bg-[#724A6A] flex items-center justify-center text-white text-[10px] font-bold">
               {initials}
             </div>
           )}
         </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className={`fixed lg:static top-14 bottom-0 left-0 z-40 w-56 bg-white border-r border-[#E8E0D0] flex flex-col py-4 transition-transform duration-300 h-full ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
...
<div className="px-3 pt-3 border-t border-[#E8E0D0] flex flex-col gap-1">
        ...
        <main className="flex-1 overflow-y-auto p-5 lg:p-8 bg-[#FFFBE9]">{children}</main>
      </div>
    </div>
  );
}
